class CeleryProgressBar {
    ...
    constructor(progressUrl, options) {
        ...
        let progressBarSubmit = options.progressBarSubmitId || 'progress-bar-submit';
        ...
        this.progressBarSubmitElement = options.progressBarSubmitElement || document.getElementById(progressBarSubmit);
    ...
    static onSuccessDefault(progressBarElement, progressBarMessageElement, result, progressBarSubmitElement) {
        progressBarElement.style.backgroundColor = '#76ce60';
        //progressBarSubmitElement.disabled = false;
        progressBarMessageElement.textContent = "Готово! " + result;
    }
    ...
    static onProgressDefault(progressBarElement, progressBarMessageElement, progress, progressBarSubmitElement) {
        progressBarElement.style.backgroundColor = '#68a9ef';
        progressBarElement.style.width = progress.percent + "%";
        var description = progress.description || "";
        if (progress.current == 0) {
            if (progress.pending === true) {
                //progressBarSubmitElement.disabled = 'true';
                progressBarMessageElement.textContent = 'Подождите, пока начнется загрузка...';
            } else {
                progressBarMessageElement.textContent = 'Загрузка началась...';
            }
        } else {
            progressBarMessageElement.textContent = 'Выполнено '+progress.current + ' из ' + progress.total + ' операций. ' + description;
        }
    }
    ...
    onData(data) {
        let done = false;
        if (data.progress) {
            this.onProgress(this.progressBarElement, this.progressBarMessageElement, data.progress, this.progressBarSubmitElement);
        }
        if (data.complete === true) {
            done = true;
            if (data.success === true) {
                this.onSuccess(this.progressBarElement, this.progressBarMessageElement, this.getMessageDetails(data.result), this.progressBarSubmitElement);
